...
export default function AnimatedGraph() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      Animated.timing(fillOpacity, { toValue: 0.18, duration: LINE_DURATION, easing: Easing.linear, useNativeDriver: false }).start();
    }, LINE_DELAY);
  ...
  // Without SkinX (red): single cubic bezier — flat then drops off smoothly
  const withoutPath = `M ${R} ${midY} C ${GW * 0.62} ${midY}, ${GW * 0.72} ${GH * 0.91}, ${GW - R} ${GH * 0.91}`;

  // Compute arc length for the red line independently (inlined bezier — no dependency on bezier fn order)
  ...
  let withoutPathLen = 0;
  {
    let prevX = wP0[0]; let prevY = wP0[1];
    for (let i = 1; i <= 300; i++) {
      const t = i / 300; const u = 1 - t;
      const x = u*u*u*wP0[0] + 3*u*u*t*wP1[0] + 3*u*t*t*wP2[0] + t*t*t*wP3[0];
      const y = u*u*u*wP0[1] + 3*u*u*t*wP1[1] + 3*u*t*t*wP2[1] + t*t*t*wP3[1];
      withoutPathLen += Math.sqrt((x - prevX) ** 2 + (y - prevY) ** 2);
      prevX = x; prevY = y;
    }
  }
        ...
        <View style={s.svgWrap}>
          <Svg width={GW + 12} height={GH} viewBox={`-6 0 ${GW + 12} ${GH}`}>
            ...
            <AnimatedPath
              ...
              strokeDashoffset={progress.interpolate({ inputRange: [0, 1], outputRange: [withoutPathLen, 0] })}
            />
